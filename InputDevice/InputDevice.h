...
{
public:
    InputDevice(WindowApplication* application);
    ~InputDevice();
    ...
    Vector2 GetMousePosition() const { return cursor_data.Position; }
    Vector2 GetMouseOffset() const   { return cursor_data.Offset; }

    Vector2 GetCalculatedMousePosition() const
    {
        POINT cursor = WindowApplication::GetCursorPosition();
        POINT center = app->GetWindowCenter();
        return { static_cast<float>(cursor.x - center.x), static_cast<float>(cursor.y - center.y) };
    }

    void SetMousePosition(Vector2 position) const
    {
        POINT point = app->GetWindowCenter();
        point.x += static_cast<int>(position.x);
        point.y += static_cast<int>(position.y);
        WindowApplication::SetCursorPosition(point);
    }
...
private:
    void SetBehaviour(MouseBehaviour* behaviour);
...
private:
    ...
    MouseBehaviour* mouse_behaviour;
